    ...
    if (argc >1) {
        if (const std::string mode=argv[1]; mode == "--file") {
            ...
            if (argc < 8) {
                ...
                std::cout<<"\tUsage:"<<std::endl<<"\t  projekt1_aizo.exe ---test <algorithm> <type> <size> <sorting> <outputFile> <numbersFile> <alg type>"<<std::endl;
                std::cout<<"\n\t<algorithm>\tSorting algorithm to use (0 - Insertion, 1 - Heap, 2 - Shell, 3 - Quick, 4 - Drunk Student)."<<std::endl;
            ...
            const std::string numbersFile = argv[7];
            //Optional arguments
            int algorithmType = -1;
            double intoxication = -1;
            if (argc == 9) {
                algorithmType = std::stoi(argv[8]);
                if (algorithm == 4) {
                    intoxication = calcIntoxication(algorithmType,static_cast<double>(std::string(argv[8]).size()));
                }
            }

            Benchmark benchmark(algorithm,type,size,sorting,outputFile,numbersFile,algorithmType,intoxication);
            benchmark.start();
